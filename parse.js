...
This is defined by expressionFeatures, and behavour can be changed by so
...
function parse(parserSteppers, grammar, textToParse, parseContext, final, timeOut) { //function start
  ...
  */
  ...
  function stepOutProcedure(f) {
    ...
    if(parseContext.stepInfo.data=="root"){return}
...
var expressionFeatures = {
  ...
  object: {
    ...
    or: function alternation(match) { //well shit
      //{type:"or",child:node[]}
      if (match.childStep) {
        if ((match.childStep.fail) && (match.context.choices.length === match.iterator)) { //none of the choices work
